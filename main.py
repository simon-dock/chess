...
def draw_system_message(player, first, promotion, castling, x, y):
...
def get_choice_promotion(x, y):
    print("get_choice_promotion")

    for i in range(4):
        x_ren = 88+116*i+10*i
        if x >= x_ren and x_ren+116 >= x and y >= 1095 and 1135 >= y:
            return i+1

    return -1

#クリックされた座標が選択肢のどの領域にあるのか取得する
def get_choice_castling(x, y):
    print("get_choice_castling")

    if x >= 88 and 88+190 >= x and y >= 1095 and 1135 >= y:
        return 0
    
    if x >= 300 and 300+163 >= x and y >= 1095 and 1135 >= y:
        return 1

    return -1
    
#座標をもとに駒の種類を取得する
def get_type_piece(x_now,y_now):
    print("get_type_piece")

    type_piece = board[y_now][x_now][1]
...
def judge_movable_position(player, x_now, y_now):
    print("judge_movable_position")
    
    init_position()

    if board[y_now][x_now][0] == -1 or board[y_now][x_now][0] != player : return False
    
    if board[y_now][x_now][0] == player:
        select_position[y_now][x_now] = 1
        movable_position[y_now][x_now] = 2
        type_piece = get_type_piece(x_now, y_now)

        search_position(player, type_piece, x_now, y_now)
...
#選ばれている駒の可動範囲を検索する
def search_position(player, type_piece, x_now, y_now):
    print("seach_position")

    if type_piece == 0:
        search_pawn(player, x_now, y_now)   
    if type_piece == 1:
        search_rook(player, x_now, y_now)
    if type_piece == 2:
        search_knight(player, x_now, y_now)
    if type_piece == 3:
        search_bishop(player, x_now, y_now)
    if type_piece == 4:
        search_queen(player, x_now, y_now)
    if type_piece == 5:
        search_king(player, x_now, y_now)
    ...
    while run:
        ...
        for event in pygame.event.get():
            ...
            if event.type == MOUSEBUTTONDOWN and event.button == 1:
                first = False
                x_row, y_row = event.pos

                if castling != 0:
                    selected_castling = get_choice_castling(x_row, y_row)
                    castling = 0
                    if selected_castling != -1:
                        player, select= do_castling(selected_castling, player, x, y)

                if promotion == True:
                    selected_piece = get_choice_promotion(x_row,y_row)
                    if selected_piece == -1:continue
                    promotion = do_promotion(selected_piece, x, y)
                else: 
                    x, y = get_position(x_row,y_row)
                    if x == -1:continue
        ...
        WIN.fill(BLACK)
        draw_board()
        draw_system_message(player, first, promotion, castling, x, y)
